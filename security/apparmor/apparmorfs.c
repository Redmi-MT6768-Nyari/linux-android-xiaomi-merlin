			      ...
			      const char *fname)
{
	char *buffer, *s;
	int error;
	/* each depth level adds "../../" (6 chars) + "raw_data/" + dirname + "/" + fname */
	size_t bsize = depth * 6 + strlen("raw_data/") + strlen(dirname) + 1 + strlen(fname) + 1;
	...
	error = snprintf(s, bsize - (s - buffer), "raw_data/%s/%s", dirname, fname);
	if (error < 0) {
		kfree(buffer);
